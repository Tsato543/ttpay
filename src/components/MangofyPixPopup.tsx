...
interface MangofyPixPopupProps {
...
const MangofyPixPopup = ({ amount, description, onSuccess, onClose }: MangofyPixPopupProps) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (stored) {
          try {
            ...
            customer = {
              name: parsed.nome || "Cliente",
              email: parsed.email || "cliente@pagamento.com",
              document: (parsed.cpf || "").replace(/\D/g, "") || "00000000000",
              phone: (parsed.telefone || "").replace(/\D/g, "") || "00000000000"
            };
          } catch (e) {
        ...
        console.log("Creating Mangofy payment:", { amount, description, customer });

        const { data, error: fnError } = await supabase.functions.invoke('mangofy-pix-create', {
          body: { amount, description, customer }
        });
    ...
    createPayment();
  }, [hasCreatedPayment, amount, description]);
    ...
    const checkStatus = async () => {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/mangofy-pix-status?id=${paymentId}`,
          {
            method: 'GET',
            headers: {
...
export default MangofyPixPopup;
